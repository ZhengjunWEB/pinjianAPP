<template>
    <div class="Mud">
        ...
        <common-nl :Index="selectedId" :nlIndex="nlIndex" @transfer_index="getUsId"></common-nl>
       
    </div>
</template>

<script>
import commonNl from "@/components/Common/Common_nl";
export default {
  name: 'Mud',
  components: {
    commonNl,
  ...
  // console.log("a:"+typeof(this.items))
    ...
    $this.$post($this.$api.getidclasschild, {
      id:344,
      no:10
    }).then(data => {
        ...
        if(data.code == 200){
         // 首页底部分类
          ...
          $this.$post($this.$api.goods_list, {
            id:data.data[0].id,
            no:10,
            istype:data.data[0].pid
          }).then(data => {
            this.nlIndex = data.data;
          });
        }
    });
  },
